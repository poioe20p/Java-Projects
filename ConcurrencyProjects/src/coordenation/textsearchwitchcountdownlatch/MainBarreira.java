...
public class MainBarreira {
	...
	public static void main(String[] args) {
		...
		CyclicBarrier barrier= new CyclicBarrier(NUM_DOCUMENTS_TO_BE_SEARCHED,new Runnable() {
			@Override
			//This run method is invoked only when all involved threads have called the barrier.await().
			//After the run method below is done, all the other involved threads are released and conclude their own run method.
			public void run() {
				int count=0;
